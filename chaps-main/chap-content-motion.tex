...
\subsection{Choosing The Supporting And Swinging Legs} \label{sec:supp_swing_calc}
    ...
    \begin{equation} 
        \mdim{m}{6}{3} = (\bm{T} - l_{str} \bm{\bm{w}_{dir}}) - \bm{t_{nom}} \label{eq:move_vec}
    \end{equation}
    where \(\bm{T}\) contains the constant rest positions the feet, \(l_{str}\) is the desired stride length, \(\bm{w}_{dir}\) contains the desired walk directions, and \(\bm{t}_{nom}\) contains the nominal targets.

    Then the optimised targets, \(\bm{t_{opt}} \inrefframe{L}\), are calculated as the addition of the previous targets and the move vector in equation \ref{eq:move_vec},
    \begin{equation} \label{eq:opt_supp}
        \mdim{t_{opt}}{6}{3} = \bm{t_{prv}} + \bm{m}
    \end{equation}
    where \(\bm{t_{prv}}\) contains the previous, optimised, targets. Note that equation \ref{eq:opt_supp} does not include the optimisation function \textcolor{red}{XXXX}, this is because the supporting feet will not
    move relative to the terrain, and thus do not need optimising.

    \subsubsection{For Swinging Legs} \label{sec:swing}
    For the supporting legs, the optimised targets in map space, \(\bm{t_{map}} \inrefframe{M}\) are calculated as in equation \ref{eq:t_map},
    \begin{equation} \label{eq:t_map}
        \mdim{t_{map}}{6}{3} = \bm{T} - \bm{\Delta \kappa} + \left(l_{str}+\avg \bm{m}\right)\bm{w_{dir}} \transframe{L}{M}
    \end{equation}
    where \(\bm{\Delta \kappa} \inrefframe{L}\) is calculated as the difference between the robot's (body's) current position and its position at the beginning of the previous step,
    \begin{equation}
        \mdim{\Delta \kappa}{1}{3} = \bm{\kappa}-\bm{\kappa_{prv}} \transframe{W}{L}
    \end{equation}
    where \(\bm{\kappa} \inrefframe{W}\) is the robot's current position, and \(\bm{\kappa_{prv}} \inrefframe{W}\) is the robot's position at the beginning of the previous step.

    Additionally, for use in equation \ref{eq:move_vec}, the swinging legs nominal targets, \(\bm{t_{nom}} \inrefframe{L}\), are calculated as in equation \ref{eq:swing_nom},
    \begin{equation} \label{eq:swing_nom}
        \mdim{t_{nom}}{6}{3} = \bm{T} - \bm{\Delta \kappa} + l_{str}\bm{w_{dir}}
    \end{equation}

    Note that \(\bm{t_{map}}\) is quite
    similar to \(\bm{t_{nom}}\), the difference is that \(\bm{t_{map}}\) is set such that it will align with \(\bm{t_{nom}}\) at the end of the step, assuming the \transframe{M}{L} transform is applied.
    Thus, the omission of the average supporting move vector, \(\avg \bm{m}\), in calculating \(\bm{t_{nom}}\).
...
\section{Kinematics}
    ...
    \subsection{\acf{ik}}
        ...
        to move the foot to the given target position vector, \(\bm{p_t} = [x_t,y_t,z_t]^T_{\displaystyle .}\)
        ...
        \end{align}
    \subsection{\acf{fk}}
        The \ac{fk} function calculates the position vector of a foot, \(\bm{p_c} = [x_c,y_c,z_c]^T\),
        given the current angles of the leg servos, \(\bm{\theta} = [\theta_1, \theta_2, \theta_3]^T\).
        \begin{align}
            \bm{p_c}(\theta_1,\theta_2,\theta_3) =
                            % \begin{bmatrix}
                            %     x_c\\
                            %     y_c\\
                            %     z_c

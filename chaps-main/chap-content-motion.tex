...
\subsection{Choosing The Supporting And Swinging Legs} \label{sec:supp_swing_calc}
    ...
    \begin{equation}
        \mdim{\Delta \kappa}{1}{3} = \bm{\kappa}-\bm{p_{prv}} \transframe{G}{L}
    \end{equation}
    where \(\bm{\kappa} \inrefframe{G}\) is the robot's current position, and \(\bm{\kappa}_{prv} \inrefframe{G}\) is the robot's position at the beginning of the previous step.
